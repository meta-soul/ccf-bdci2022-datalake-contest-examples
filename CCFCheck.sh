...
tar xf target/datalake.tar.gz -C work-dir
# 如果数据不存在，则下载并解压数据
...
# 清理 S3(Minio) 数据
docker run --net lakesoul-compose-env_default --rm -t swr.cn-north-4.myhuaweicloud.com/dmetasoul-repo/spark:v3.1.2 aws --no-sign-request --endpoint-url http://minio:9000 s3 rm --recursive s3://ccf-datalake-contest/
# 清理结果数据
docker run --net lakesoul-compose-env_default --rm -t -v ${PWD}/work-dir:/opt/spark/work-dir swr.cn-north-4.myhuaweicloud.com/dmetasoul-repo/spark:v3.1.2 rm -rf /opt/spark/work-dir/result
